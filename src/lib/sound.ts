 ...
 * 把握しているため、**再生開始からの経過時間（`SOUND_CLEANUP_MS`）で
 * 破棄する**方式にした。実際の音の長さより余裕を持たせてあるため、
 * 破棄が早すぎて音が途切れることはない。
 *
 * [2026-09-16改訂・実装メモ220.9章] 音源を系統A2（実装メモ220.9章参照。
 * `tools/generate_sounds.js`の既定出力）に差し替えたことに伴い、各音の長さが
 * 0.45〜0.6秒から0.75〜0.95秒に伸びたため、`SOUND_CLEANUP_MS`も合わせて
 * 見直した（詳細は同定数の直上コメント）。
 */
...
/**
 * 各音源の実際の長さ＋余裕（ミリ秒）。`tools/generate_sounds.js`（既定＝系統A2）が
 * 生成する長さ（report=0.75s／reward=0.85s／gacha=0.95s）に対し、機種差・
 * バッファ分の余裕として+400msを足した値にしている（早く破棄して音が途中で
 * 切れることを防ぐのが目的で、破棄が多少遅れても副作用は無い＝安全側に倒す
 * ほうがよいため、220.2章時点の旧音（差分約450ms）と同程度の余裕を踏襲した）。
 * この時間が経過したら再生済みとみなしてプレイヤーを破棄する。
 *
 * [2026-09-16改訂・実装メモ220.9章] 音源をA2に差し替え、長さが0.45〜0.6秒から
 * 0.75〜0.95秒に伸びたことに合わせて値を更新した（900/1050/1000 →
 * 1150/1250/1350）。
 *
 * [2026-09-25改訂・実装メモ297章] 音源を「やさしい版」に差し替えた（A2が
 * 緊急地震速報に似ていると家族に言われたため）。長さは完了報告0.6秒・
 * ごほうび交換1.05秒・ガチャ0.85秒。同じく400msの余裕を足した。
 */
const SOUND_CLEANUP_MS: Record<SoundName, number> = {
  report: 1000, // 0.60s + 400ms
  reward: 1450, // 1.05s + 400ms
  gacha: 1250, // 0.85s + 400ms
};
