            ...
            <Card key={c.id} style={{ ...styles.row, ...(highlighted ? styles.rowHighlighted : null) }}>
              <Pressable disabled={done || !me} onPress={goToReport} style={styles.rowMain}>
                ...
                {/* [2026-09-22改訂・要件定義書07-31章「2026-09-22改訂」決定6、主要画面
                    ワイヤーフレーム.md 53.11.11節決定13・53.11.9節2] P19は決定6により
                    決定8（誰でも実行可→家族合計）の対象外に戻った。担当設定に関わらず、
                    常に本人（me.id）の回数「・計◯回」を表示する（2026-09-20の決定8適用前と
                    同じ挙動）。取得に失敗したときは回数の部分だけ出さない（53.7節）。 */}
                {done ? (
