...
/**
 * S4 一緒にやることの完了報告（みまもりメンバー）
 * 参照: 画面一覧・遷移図.md 2.5節S4・3.12節、API仕様.md 4c章
 *
 * app/parent/my-chore-report.tsx（P20）と同じ`chore_completions` insert
 * （API呼び出し自体は保護者・子どもと完全に同一。API仕様.md 4c章「呼び出し方自体は
 * 保護者・子どもと全く同一」）。3回目のスコープ変更により通常どおりポイントが
 * 即時付与される。
 */
type ScreenState = "form" | "uploadingPhoto" | "sending" | "networkError" | "limitReached";

export default function SharedChoreReportScreen() {
  ...
  if (!chore || !me) {
    return (
      <Screen tone="supporter">
        ...
        <AppButton tone="supporter" label="いっしょにやることリストへもどる" onPress={() => router.replace("/supporter/shared-chores")} />
      </Screen>
    );
  }
  ...
  const send = async () => {
    ...
    router.replace({
      pathname: "/supporter/shared-chores",
    ...
    return (
      <Screen tone="supporter">
        ...
        <View style={styles.centerBlock}>
          <Text style={theme.typography.supporterBodyMedium}>本日はすでにいっしょにやりました</Text>
        </View>
        <AppButton tone="supporter" label="いっしょにやることリストへもどる" fullWidth onPress={() => router.replace("/supporter/shared-chores")} />
      </Screen>
    );
  }
