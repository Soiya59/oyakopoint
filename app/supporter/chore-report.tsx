 ...
 * 参照: 画面一覧・遷移図.md 2.5節S7、API仕様.md 4c章・4c-2章
 *
 * 通常どおりポイントが表示・付与される点でS4（家族共有への参加）と対象範囲が異なるだけで
 * APIロジックは同一（呼び出し方自体は保護者・子どもと全く同一。4c章・4c-2章参照）。
 *
 ...
 *
 * [2026-09-06改訂・要件定義書07-18章決定2・12・UIUXデザイン部30.4節決定12]
 * このコンポーネントは`created_by`を一切参照せず`isChoreLimitReached`のみで
 * ボタン活性・上限到達を判定しているため、scope='supporter_shared'（作成者を
 * 問わずみまもりメンバー全員が完了報告できる）にもそのまま乗る。変更不要。
 */
  ...
  if (screenState === "networkError") {
    return (
      <Screen tone="supporter">
        ...
        <View style={styles.centerBlock}>
          <Text style={theme.typography.supporterBody}>{sendErrorMessage ?? GENERIC_ERROR_MESSAGE}</Text>
        </View>
        <AppButton
          tone="supporter"
          label="もう一度送信する"
